...
class PieChart extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    //size는 원의 중앙 정렬
    drawText(canvas, Size(150.0, 130.0), "$remain", 1.0, Colors.orangeAccent);
    drawText(canvas, Size(150.0, 180.0), "잔여 칼로리", 0.6, Colors.black87);
  }

  // 원의 중앙에 텍스트를 적음.
  void drawText(Canvas canvas, Size size, String text, double textScaleFactor, Color color){
    double fontSize = getFontSize(size, text, textScaleFactor);

    TextSpan sp = TextSpan(style: TextStyle(fontSize: fontSize, fontWeight: FontWeight.bold, color: color), text: text); // TextSpan은 Text위젯과 거의 동일하다. 
